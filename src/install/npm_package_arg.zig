...
pub const NpaSpec = struct {
    ...
    const StringArena = struct {
        ...
        fn copyString(self: *StringArena, str: []const u8) []const u8 {
            if (self.offset + str.len > self.buffer.len) {
                @panic("StringArena buffer overflow: arena size miscalculation");
            }
    ...
    fn fromUrl(
        ...
        const protocol_slice = try url.Slice.protocol(parsed_url, allocator);
        ...
        switch (protocol_type) {
            .http, .https => {
                ...
                spec._fetch_spec_slice = raw_spec_slice;
                spec.save_spec = raw_spec_slice;
                spec.type = .remote;
            },
    ...
    fn fromRegistry(
        ...
        const query = Semver.Query.parse(
            ...
            try validateNoURLEncoding(trimmed);
            res.type = .tag;
            return res;
        };
        defer query.deinit();
    ...
    fn normalizePath(
        ...
        {
            const home_dir = try bun.os.HomeDir.query(allocator, .{});
            defer _ = home_dir.deinit();
            const path_after_tilde = if (spec_path.len > 3) spec_path[3..] else "";
            const resolved = try PathHelpers.resolve(&.{ home_dir.slice(), path_after_tilde }, &path_buffers);
...
const GitAttrs = struct {
    ...
    pub fn fromUrl(allocator: std.mem.Allocator, parsed_url: *bun.jsc.URL) !?Self {
        const hash_slice = try url.Slice.hash(parsed_url, allocator);
...
fn validateNoURLEncoding(str: []const u8) !void {
    if (bun.strings.indexOfNeedsURLEncode(str) != null) {
...
const SpecStrUtils = struct {
    ...
    pub fn getUrlHrefWithoutHash(allocator: std.mem.Allocator, parsed_url: *bun.jsc.URL) ![]u8 {
        const href_slice = try url.Slice.href(parsed_url, allocator);
    ...
    pub fn extractHostAndPathnameWithLowercaseHost(
        ...
        const host_slice = try url.Slice.host(parsed_url, allocator);
        defer allocator.free(host_slice);
        const pathname_slice = try url.Slice.pathname(parsed_url, allocator);
